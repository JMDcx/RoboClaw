...
@pytest.mark.asyncio
async def test_g1_setup_action() -> None:
    tool = EmbodiedTool()
    with patch("roboclaw.embodied.setup.set_unitree_g1") as set_g1, patch("roboclaw.embodied.setup.clear_unitree_g1"):
        ...
        result = await tool.execute(action="g1_setup", network_interface="wlp68s0", dds_domain=1)
    assert "Unitree G1 simulation configured" in result
    set_g1.assert_called_once()


@pytest.mark.asyncio
async def test_g1_connect_status_and_move_actions() -> None:
    tool = EmbodiedTool()
    tool._g1_controller = SimpleNamespace(
        connect=AsyncMock(return_value={"ok": True, "command_topic": "rt/lowcmd"}),
