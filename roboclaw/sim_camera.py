...
class IsaacSimCameraReader:
    """Read one RGB camera stream from Isaac Lab shared memory."""
    ...
    def read_latest_frame(self) -> SimCameraFrame:
        """Read and decode the latest frame from shared memory."""
        shm = self._open_shm()
    ...
    def save_latest_frame(self, output_dir: Path) -> tuple[SimCameraFrame, Path]:
        """Read the latest frame and save it as a PNG file."""
